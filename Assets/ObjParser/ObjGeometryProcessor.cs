            ...
            for (int j = 0; j < DataProcess.datList.Count; j++)
            {
                ...
                foreach (var pair in DataProcess.triangleList)
                {
                    ...
                    if (primType == "triangle")
                    {
                        ...
                        {
                            colors[j][pair.Value[i] - 2] = gradient.Evaluate(DataProcess.datList[j][id]);
                            colors[j][pair.Value[i] - 1] = gradient.Evaluate(DataProcess.datList[j][id]);
                            colors[j][pair.Value[i]] = gradient.Evaluate(DataProcess.datList[j][id]);
                            id++;
                        }
                    }
                    else
